...
def compile_extension():
    cuda_source = Path(
        "/workspace/Custom-Kernel-Impl/rope/rope_kernel.cu"
    ).read_text()
    cpp_source = "void rope(torch::Tensor& data, torch::Tensor& sin, torch::Tensor& cos, const int32_t head_dim, const int32_t head_num, const int32_t token_num);"
...
def apply_rope(input_tensor, sin, cos, head_dim=128):
    ...
    rotated_tensor = torch.cat((-input_tensor[:, embed_dim:], input_tensor[:, :embed_dim]), dim=1)  # [num_tokens, head_dim]
    results[:, embed_dim:] = input_tensor[:, :embed_dim] * sin + input_tensor[:, embed_dim:] * cos
    results[:, :embed_dim] = rotated_tensor[:, :embed_dim] * sin + rotated_tensor[:, embed_dim:] * cos
    return results
...
def rope_base_impl(input_tensor, sin, cos, head_dim=128, head_num=32):
    ...
    for head_idx in range(head_num):
        results[:, head_dim * head_idx : head_dim * (head_idx + 1)] = apply_rope(input_tensor[:, head_dim * head_idx : head_dim * (head_idx + 1)], sin, cos, head_dim)
    return results
